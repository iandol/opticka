...
StimLevels = linspace(minStim,maxStim,9);

%Number of trials at each entry of 'StimLevels'
OutOfNum = ones(size(StimLevels))*100;

%Number of positive responses (e.g., 'yes' or 'correct' at each of the 
%   entries of 'StimLevels'  
NumPos = PAL_PF_SimulateObserverParametric([maxStim/2 10.^beta gamma lambda],StimLevels,OutOfNum,PF);
...
%estimates.
grid.alpha = linspace(0,maxStim+(maxStim/100),201);
grid.beta = linspace(0,3,181);  %log-transformed values for beta
grid.gamma = gamma;               %using fixed value for guess rate ...
grid.lambda = lambda;             %... and lapse rate

%Define a prior distribution across parameter space (optional).
[a b g l] = ndgrid(grid.alpha,grid.beta,grid.gamma,grid.lambda);
prior = PAL_pdfNormal(a,maxStim/2,maxStim/4).*PAL_pdfNormal(b,beta,beta/2);
prior = prior./sum(sum(sum(sum(prior))));
figure;imagesc(grid.beta,grid.alpha,prior);colorbar
%Fit function
[paramsValues2D posterior2D] = PAL_PFBA_Fit(StimLevels, NumPos, OutOfNum, grid, PF,'prior',prior);
figure;imagesc(grid.beta,grid.alpha,posterior2D);colorbar

%Fit again using free lapse rate
grid.lambda = [0:.001:.06];
% %Define a prior distribution across parameter space (optional)
[a b g l] = ndgrid(grid.alpha,grid.beta,grid.gamma,grid.lambda);
prior = PAL_pdfNormal(a,maxStim/2,maxStim/4).*PAL_pdfNormal(b,beta,beta/2).*l.^2.*(1-l).^98; %last two terms define beta distribution (minus normalization) with mode 0.02 on lapse rate
...
ax2 = axes;
hold on;
plot(gca, minStim:maxStim/1000:maxStim,PF([paramsValues2D(1,1) 10.^paramsValues2D(1,2) paramsValues2D(1,3) paramsValues2D(1,4)],minStim:maxStim/1000:maxStim),'-','color',[0 .7 0],'linewidth',2) 
plot(gca, StimLevels,NumPos./OutOfNum,'ko','markersize',10,'markerfacecolor','k');
set(gca, 'units','pixels','position',[500 75 275 150]);
%set(gca,'xlim',[0 .12],'ylim',[0 1],'xtick',StimLevels,'ytick',[0 0.5 1.0])
