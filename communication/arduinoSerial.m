classdef arduinoSerial < handle
	...
	methods
		...
		function pinMode(a,pin,str)
			...
			if a.chkp
				...
				if nargin>2
					errstr=arduinoSerial.checkstr(str,'pin mode',{'input','output'});
					if ~isempty(errstr), error(errstr); end
				end
		...
		function digitalWrite(a,pin,val)
			...
			if a.chkp
				...
				if ~isempty(errstr), error(errstr); end
				% check val
				errstr=arduinoSerial.checknum(val,'value',0:1);
				if ~isempty(errstr), error(errstr); end
				% get object name
		...
		function analogWrite(a,pin,val)
			...
			if a.chkp
				...
				% check pin
				errstr=arduinoSerial.checknum(pin,'pwm pin number',[2:13 44:46]);
				if ~isempty(errstr), error(errstr); end
				
				% check val
				errstr=arduinoSerial.checknum(val,'analog output level',0:255);
				if ~isempty(errstr), error(errstr); end
		...
		function analogReference(a,str)
			...
			if a.chkp,
				...
				% check val
				errstr=arduinoSerial.checkstr(str,'reference',{'default','internal','external'});
				if ~isempty(errstr), error(errstr); end
		...
		function servoWrite(a,pin,val)
			...
			if a.chkp,
				...
				% check angle value
				errstr=arduinoSerial.checknum(val,'angle',0:180);
				if ~isempty(errstr), error(errstr); end
		...
		function encoderAttach(a,enc,pinA,pinB)
			...
			if a.chkp,
				...
				errstr=arduinoSerial.checknum(enc,'encoder number',0:2);
				if ~isempty(errstr), error(errstr); end
				
				% check pin A
				errstr=arduinoSerial.checknum(pinA,'pin A',[2 3 19 18 21 20]);
				if ~isempty(errstr), error(errstr); end
				
				% check pin B
				errstr=arduinoSerial.checknum(pinB,'pin B',[2 3 19 18 21 20]);
				if ~isempty(errstr), error(errstr); end
		...
		function encoderDebounce(a,enc,del)
			...
			if a.chkp,
				...
				% check delay value
				errstr=arduinoSerial.checknum(del,'del',0:69);
				if ~isempty(errstr), error(errstr); end
		...
		function val=roundTrip(a,byte)
			...
			if a.chkp
				...
				errstr=arduinoSerial.checknum(byte,'byte',0:255);
				if ~isempty(errstr), error(errstr); end
		...
		function val=motorSpeed(a,num,val)
			...
			if a.chkp,
				...
				if nargin>1,
					errstr=arduinoSerial.checknum(num,'motor number',1:4);
					if ~isempty(errstr), error(errstr); end
				end
				
				% if speed argument is there check it
				if nargin>2,
					errstr=arduinoSerial.checknum(val,'speed',0:255);
					if ~isempty(errstr), error(errstr); end
				end
		...
		function motorRun(a,num,dir)
			...
			if a.chkp,
				errstr=arduinoSerial.checkstr(dir,'direction',{'forward','backward','release'});
				if ~isempty(errstr), error(errstr); end
			end
		...
		function val=stepperSpeed(a,num,val)
			...
			if a.chkp,
				...
				if nargin>1,
					errstr=arduinoSerial.checknum(num,'stepper number',1:2);
					if ~isempty(errstr), error(errstr); end
				end
		...
		function stepperStep(a,num,dir,sty,steps)
			...
			if a.chkp,
				...
				if nargin>3,
					% check direction
					errstr=arduinoSerial.checkstr(sty,'motion style',{'single','double','interleave','microstep'});
					if ~isempty(errstr), error(errstr); end
				else
					sty='single';
				end
				...
				if nargin==5,
					errstr=arduinoSerial.checknum(steps,'number of steps',0:255);
					if ~isempty(errstr), error(errstr); end
				else
					steps=0;
				end
	...
	methods (Static) % static methods
		...
		function errstr=checknum(num,description,allowed)
			...
			% errstr=arduinoSerial.checknum(num,description,allowed); Checks numeric argument.
		...
		function errstr=checkstr(str,description,allowed)
			...
			% errstr=arduinoSerial.checkstr(str,description,allowed); Checks string argument.
		...
		function errstr=checkser(ser,chk)
			...
			% errstr=arduinoSerial.checkser(ser,chk); Checks serial connection argument.
