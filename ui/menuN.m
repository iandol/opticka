...
%% menuN: An alternative to Matlab's menu function with some added functionality
%   
% Syntax:
...
%           Start an option string part with ¤ to have it defualt toggled on.     
%           |--mtitle---------------|
%           |  | options part 1 |   |
%           |  | options part 2 |   |
%           |  |       ...      |   |
...
%
% Output:
%  choice   - [double || cell of doubles] - selected option(s)
...
if nargin == 0
   ...
   mtitle   = 'This is the menuN test example settings:';
   options  = {  [1,8,1.75], 'Please Enter Loss Parameter:';   ...
          'r|optA1|op2|¤defasdfasdfa','Choose Operating Model:';...
          'p|a|b|¤c|','Select a, b or c:';...
			 'one|two|¤three|four','Please select from the list of choices presented';...
          'x|Use1|¤Use2|¤Use3','Use methods:';...
          't|first test','Comment:'};
   Opt      = defOpt;
elseif nargin == 3
   if isstruct(Opt)
...
extentWidthUniversal          = 300;   % It is increased/decreased when necessary
...
hFig           = figure('Name',mtitle,'Toolbar','none','Menubar','none','NumberTitle','off','Resize','on');
   ...
   elseif ischar(tmpOptions) && strcmp(tmpOptions(1:2),'r|')   
      ...
      if ~isempty(tmpMarkedIdx)
         ...
         if length(tmpValue) > 1
            warning('menuN:popupmenu:Only one selected element is valid as default.');
            tmpValue = tmpValue(1);
         end
...
% Override the close request function to simply resume running this code:
set(hFig,'CloseRequestFcn',{@closeMenuFigure,hFig,-1});
...
%% Close request function for menu figure
