function rc = eyelinkCallback(callArgs, msg)
	...
	if verbose; fprintf('--->>> EYELINKCALLBACK RUNNING: drawcount=%g\n', drawcount); end
	...
	newcamimage = 0;
	needsupdate = 0;
	if verbose; fprintf('--->>> EYELINKCALLBACK eyecmd:%g: drawcount=%g\n', eyecmd, drawcount); end
	switch eyecmd
		...
		case 3,
			% Alert message:
			if verbose; fprintf('--->>> EYELINKCALLBACK Eyelink Alert: %s.\n', msg); end
			needsupdate = 1;
			% TODO FIXME: Implement some reasonable behaviour...
		case 4,
			% Image title of camera image transmitted from Eyelink:
			if verbose; fprintf('--->>> EYELINKCALLBACK Eyelink image title is %s. [Threshold = %f]\n', msg, callArgs(2)); end
			if callArgs(2) ~= -1
		...
		case 5,
			...
			if verbose; fprintf('--->>> EYELINKCALLBACK draw_cal_target.\n'); end
			calxy = callArgs(2:3);
			clearScreen=1;
			needsupdate = 1;
		...
		case 6,
			...
			if verbose; fprintf('--->>> EYELINKCALLBACK clear_cal_display.\n'); end
			clearScreen=1;
			drawInstructions=1;
			needsupdate = 1;
		...
		case 8,
			...
			if verbose; fprintf('--->>> EYELINKCALLBACK setup_image_display for %i x %i pixels.\n', eyewidth, eyeheight); end
			drawcount = 0;
			lastImageTime = GetSecs;
		...
		case 9,
			% Exit image display:
			if verbose; 
				fprintf('--->>> EYELINKCALLBACK exit_image_display.\n');
		...
		case 10,
			...
			if verbose; fprintf('--->>> EYELINKCALLBACK erase_cal_target.\n'); end
			calxy = [];
			clearScreen=1;
			needsupdate = 1;
		case 11,
			if verbose; 
				fprintf('--->>> EYELINKCALLBACK exit_cal_display.\n');
				fprintf('--->>> EYELINKCALLBACK AVG FPS = %f Hz\n', drawcount / (GetSecs - lastImageTime));
			end
			clearScreen=1;
			%       drawInstructions=1;
		...
		case 12,
			...
			if verbose; fprintf('--->>> EYELINKCALLBACK cal_target_beep_hook.\n'); end
			EyelinkMakeSound(el, 'cal_target_beep');
		case 13,

			% New drift correction target sound:
			if verbose; fprintf('--->>> EYELINKCALLBACK dc_target_beep_hook.\n'); end
			EyelinkMakeSound(el, 'drift_correction_target_beep');
		case 14,
			% Calibration done sound:
			errc = callArgs(2);
			if verbose; fprintf('--->>> EYELINKCALLBACK cal_done_beep_hook: %i\n', errc); end
			if errc > 0
				% Calibration failed:
		...
		case 15,
			...
			errc = callArgs(2);
			if verbose; fprintf('--->>> EYELINKCALLBACK dc_done_beep_hook: %i\n', errc); end
			if errc > 0
...
function EyelinkMakeSound(el, s)
	...
	if doBeep==1
		Beeper(f, v, d);
		if verbose; fprintf('Beeper was run...'); end
	end
end
...
function EyelinkDrawCalibrationTarget(eyewin, el, calxy)
	...
	if verbose; fprintf('--->>> EYELINKCALLBACK Draw Calibration Target \n'); end
