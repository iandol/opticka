function ianprocedural(benchmark, nonsymmetric)
...
Screen('BlendFunction', win, 'GL_SRC_ALPHA', 'GL_ZERO');
tw = res(1);
th = res(2);
x=tw/2;
y=th/2;
...
Screen('DrawTexture', win, gabortex, [], [], tilt, [], [], [], [], kPsychDontDoRotation, [phase, freq, sc, contrast, aspectratio, 0, 0, 0]);
...
if benchmark == 2
	...
	set(h2,'Position',[600 100 500 500])
end
...
while count < 10000
    ...
    if benchmark == 2
        ...
        a=cos(deg2rad(tilt+90))*sf*360;
        b=sin(deg2rad(tilt+90))*sf*360;
	...
	Screen('gluDisk', win, [255,255,255,50]', 600, 600, 100);
    Screen('DrawTexture', win, gabortex, [], [], tilt, [], [], [], [], kPsychDontDoRotation, [phase freq sc contrast, aspectratio, 0, 0, 0]);
	    if benchmark > 0
